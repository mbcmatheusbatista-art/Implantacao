...
function captureFieldLabel(kind: ImportKind, field: FieldKey) {
  ...
  if (kind === "calls" && field === "chamado") return "Chamado (nº do chamado)";
  if (kind === "calls" && field === "atendente") return "Atendente (quem atende)";
  if (kind === "calls" && field === "fat") return "Faturamento (FAT)";
...
export function ImportDialog({
  ...
  const pastedCapturePreview = useMemo(() => {
    ...
    const pastedHeaders = (rows[detectedHeaderRow] ?? []).map(
  ...
  function handleConfirm() {
    const directPasteRows =
      rawRows.length > 0 ? rawRows : (pastedRowsWithFormatting ?? parsePastedData(pastedText));
    const effectiveHeaderRow =
      rawRows.length > 0 ? headerRow : detectHeaderRow(directPasteRows, kind);
    const effectiveDataObjects =
      rawRows.length > 0 ? dataObjects : rowsToObjects(directPasteRows, effectiveHeaderRow).data;
    ...
    if (missing.length > 0) {
      toast.error(
        `Escolha uma coluna para: ${missing.map((field) => FIELD_LABELS[field]).join(", ")}.`,
      );
      return;
    }
  ...
  const previewHeaders: string[] = allFields.map((f) => mapping[f]).filter((h): h is string => !!h);
              ...
              {pastedText.trim().length > 0 && rawRows.length === 0 && (
                <div className="rounded-md border border-primary/30 bg-primary/5 px-3 py-2 text-sm text-primary animate-in fade-in slide-in-from-left-2 duration-500">
                  <div className="flex items-center gap-2">
                    ...
                    <span>
                      Dados colados. Clique em <strong>“Analisar dados”</strong> para verificar as
                      colunas capturadas e ajustá-las, se necessário.
                    </span>
                  </div>
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {requiredFields.map((field) => (
                      <span
                        key={field}
                        className={`rounded-full border px-2 py-0.5 text-xs ${pastedCapturePreview[field] ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-amber-200 bg-amber-50 text-amber-800"}`}
                      >
          ...
          {rawRows.length > 0 && (
            <div className="space-y-3">
              ...
              <div className="rounded-lg border bg-muted/30 p-3 space-y-3">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Escolha abaixo qual coluna da planilha preenche cada campo. O nome do cabeçalho
                    pode ser diferente: selecione a coluna que contém os dados corretos nas linhas.
                    A seleção será usada em todo o sistema após a importação.
                  </p>
                </div>
                <div className="flex flex-wrap gap-2">
                  {requiredFields.map((field) => (
                    <span
                      key={`required-${field}`}
                      className="rounded bg-foreground px-1.5 py-0.5 text-[11px] font-semibold uppercase tracking-wide text-background"
                    >
                      {FIELD_LABELS[field]}
                    </span>
                  ))}
                ...
                <div className="flex flex-wrap gap-2">
                  {requiredFields.map((field) => (
                    <span
                      key={field}
                      className={`rounded-full border px-2 py-1 text-xs ${mapping[field] ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-amber-200 bg-amber-50 text-amber-800"}`}
                    >
                      {mapping[field]
                        ? `${captureFieldLabel(kind, field)} capturando a coluna “${mapping[field]}”`
                        : `${captureFieldLabel(kind, field)}: escolha uma coluna`}
                    </span>
                  ))}
                </div>
              </div>
              ...
              <p className="hidden text-xs text-muted-foreground">
                Escolha as colunas que deseja capturar. A prÃ©-visualizaÃ§Ã£o e a importaÃ§Ã£o
                usarÃ£o somente estas colunas.
              </p>
              ...
              {optionalFields.length > 0 && (
                <details className="rounded-md border px-3 py-2">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    Use somente se sua planilha também possuir essas informações.
                  </p>
