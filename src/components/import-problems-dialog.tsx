...
type ImportRecord = InitialContact | ConfirmedService | Technician | ServiceCall;
...
const FIELDS: Record<ImportKind, Field[]> = {
  ...
  calls: [
    { key: "chamadoOriginal", label: "Chamado" },
    { key: "plateOriginal", label: "Placa" },
    { key: "equipmentOriginal", label: "Equipamento" },
    { key: "atendenteOriginal", label: "Atendente" },
    { key: "fatOriginal", label: "Faturamento" },
  ],
};

function recordName(kind: ImportKind, record: ImportRecord) {
  if (kind === "technicians") return (record as Technician).nameOriginal;
  if (kind === "calls") return (record as ServiceCall).chamadoOriginal;
...
function updateRecord(
  ...
  if (kind === "calls") {
    const item = next as ServiceCall;
    item.plateNormalized = item.plateOriginal.replace(/[^a-z0-9]/gi, "").toUpperCase();
    item.equipmentNormalized = normalizeEquipment(item.equipmentOriginal);
    item.validationIssues = [
      ...(!item.chamadoOriginal.trim() ? ["Chamado vazio"] : []),
      ...(!item.plateOriginal.trim() ? ["Placa vazia"] : []),
      ...(!item.atendenteOriginal?.trim() ? ["Atendente vazio"] : []),
      ...(item.equipmentNormalized === "NAO_IDENTIFICADO" ? ["Equipamento não identificado"] : []),
    ];
    return next;
  }

  const withPhone = next as InitialContact | ConfirmedService | Technician;
  const phone = normalizeBrazilianPhone(values.phoneOriginal ?? withPhone.phoneOriginal);
  withPhone.phoneOriginal = values.phoneOriginal ?? withPhone.phoneOriginal;
  withPhone.phoneNormalized = phone.primary;
  ...
  if (kind === "initial") {
    ...
    item.validationIssues = [
      ...
      ...(normalizeEquipment(item.equipmentOriginal) === "NAO_IDENTIFICADO"
  ...
  useEffect(() => {
    ...
    setDraft(
      Object.fromEntries(
        FIELDS[kind].map(({ key }) => [
          key,
          String((selected as unknown as Record<string, unknown>)[key] ?? ""),
        ]),
      ),
    );
  }, [kind, selected]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[85vh] overflow-hidden p-0 gap-0">
        <DialogHeader className="px-6 pt-6 pb-4 border-b">
          ...
          <DialogDescription>
            Veja o motivo indicado em cada linha, ajuste os dados e salve. Você também pode excluir
            ou renomear o registro.
          </DialogDescription>
        </DialogHeader>
        {problems.length === 0 ? (
          <div className="p-8 text-center text-muted-foreground">
            Não há mais registros com problemas neste card.
          </div>
        ) : (
              ...
              {problems.map((record) => (
                ...
                >
                  <p className="font-medium text-sm truncate">
                    {recordName(kind, record) || "Sem nome"}
                  </p>
                  <p className="mt-1 text-xs text-destructive line-clamp-2">
                    {record.validationIssues.join(" • ")}
                  </p>
                </button>
              ))}
            </aside>
            {selected && (
              <section className="overflow-y-auto p-5 space-y-5">
                <div className="rounded-md border border-destructive/30 bg-destructive/5 p-3">
                  <p className="flex items-center gap-2 font-medium text-sm text-destructive">
                    <AlertCircle className="w-4 h-4" /> O que precisa ser corrigido
                  </p>
