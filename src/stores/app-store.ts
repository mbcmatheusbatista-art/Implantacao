...
interface AppState {
  ...
  setCalls: (r: ServiceCall[], meta: ImportMeta, diag: ImportDiagnostic) => void;
  replaceImportRecords: (
    kind: ImportKind,
    records: (InitialContact | ConfirmedService | Technician | ServiceCall)[],
  ) => void;
...
export const useAppStore = create<AppState>((set) => ({
  ...
  setCalls: (r, meta, diag) => {
    set((s) => ({
      calls: r,
      meta: { ...s.meta, calls: meta },
      diagnostics: { ...s.diagnostics, calls: diag },
    }));
    saveToDb("serviceCalls", r).catch(() => {});
    saveToDb("diagnostics", { calls: diag }).catch(() => {});
  },
  replaceImportRecords: (kind, records) => {
    set((s) => {
      const count = records.length;
      const nextMeta = s.meta[kind] ? { ...s.meta, [kind]: { ...s.meta[kind]!, count } } : s.meta;
      if (kind === "initial")
        return { initialContacts: records as InitialContact[], meta: nextMeta };
      if (kind === "confirmed")
        return { confirmedServices: records as ConfirmedService[], meta: nextMeta };
      if (kind === "calls") return { calls: records as ServiceCall[], meta: nextMeta };
...
export async function hydrateFromDb(): Promise<void> {
  try {
    ...
    if (data.serviceCalls) toSet.calls = data.serviceCalls;
    if (data.assignments) toSet.assignments = data.assignments;
    if (data.contactedIds) toSet.contactedIds = new Set(data.contactedIds);
    if (data.diagnostics)
      toSet.diagnostics = data.diagnostics as Partial<Record<ImportKind, ImportDiagnostic>>;
