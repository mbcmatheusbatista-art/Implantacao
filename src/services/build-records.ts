...
export function buildServiceCalls(
  rows: Record<string, string>[],
  mapping: Partial<Record<FieldKey, string>>,
  headerRow: number,
): BuildResult<ServiceCall> {
  debugBuild("calls:start", {
    totalRows: rows.length,
    mapping,
    headerRow,
    sampleRows: rows.slice(0, 5),
  });
  const records: ServiceCall[] = [];
  let imported = 0;
  let skipped = 0;
  let emptyChamados = 0;
  let emptyPlates = 0;
  let emptyAtendentes = 0;
  let equipmentUnknown = 0;

  rows.forEach((row, idx) => {
    const chamadoRaw = stripFormatMarkers(getField(row, mapping, "chamado").trim());
    const plateRaw = stripFormatMarkers(getField(row, mapping, "plate").trim());
    const equipmentRaw = stripFormatMarkers(getField(row, mapping, "equipment").trim());
    const atendenteRaw = stripFormatMarkers(getField(row, mapping, "atendente").trim());
    const fatRaw = stripFormatMarkers(getField(row, mapping, "fat").trim());
    debugBuild("calls:row:raw-fields", {
      index: idx,
      spreadsheetRow: idx + headerRow + 2,
      row,
      chamadoRaw,
    ...
    if (!chamadoRaw && !plateRaw && !atendenteRaw && !fatRaw) {
      skipped++;
      debugBuild("calls:row:skipped-empty", { index: idx, row });
      return;
    }
    const eq = normalizeEquipment(equipmentRaw);
    const issues: string[] = [];
    if (!chamadoRaw) {
      issues.push("Chamado vazio");
      emptyChamados++;
    }
    if (!plateRaw) {
      issues.push("Placa vazia");
    ...
    const record: ServiceCall = {
      id: uid(),
      chamadoOriginal: chamadoRaw,
      plateOriginal: plateRaw,
      plateNormalized: normalizePlate(plateRaw),
      equipmentOriginal: equipmentRaw,
      equipmentNormalized: eq,
      atendenteOriginal: atendenteRaw || undefined,
      fatOriginal: fatRaw || undefined,
      validationIssues: issues,
    };
    debugBuild("calls:row:created", {
      index: idx,
      spreadsheetRow: idx + headerRow + 2,
      record,
      issues,
  ...
  return {
    ...
    diagnostic: {
      ...
      invalidPhones: 0,
      emptyPlates,
      emptyNames: emptyChamados,
      emptyAddresses: emptyAtendentes,
      equipmentUnknown,
      quantityUnparsed: 0,
      groupedContacts: 0,
      nameConflicts: 0,
...
export function buildTechnicians(
  ...
  rows.forEach((row, idx) => {
    ...
    const duplicateKey =
      normalizedNameKey && contactKey ? `${normalizedNameKey}::${contactKey}` : "";
    ...
    if (duplicateKey && seen.has(duplicateKey)) {
      ...
      if (qRaw) {
        ...
        if (
          ...
          qClean !== "0" &&
          qClean !== "—" &&
          !existing.quantityOriginal.includes(qClean)
        ) {
          existing.quantityOriginal =
            existing.quantityOriginal && existing.quantityOriginal !== "—"
      ...
      if (eqBreakdown) {
        if (!existing.equipmentBreakdown) {
          ...
          for (const [eqKey, eqVal] of Object.entries(eqBreakdown)) {
            existing.equipmentBreakdown[eqKey] =
              (existing.equipmentBreakdown[eqKey] ?? 0) + (eqVal ?? 0);
          }
        }
      }
