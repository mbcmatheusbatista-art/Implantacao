...
const SEED: SeedItem[] = [
  ...
  },
  {
    ...
    address:
      "Rua Padre Manuel da Nóbrega, nº 424, Apartamento 101, Bloco 1, Fanny, Curitiba - PR, CEP 81030-330",
    lat: -25.4822762,
    lng: -49.2731379,
  },
  {
  ...
  },
  {
    ...
    address:
      "Av. Mauro Miranda Madureira, 562 - Teixeira Leite, Cachoeiro de Itapemirim - ES, 29310-290",
    lat: -20.8257949,
    lng: -41.1358825,
  },
  {
  ...
  },
  {
    firstName: "joão",
    keywords: [
      "JOAO EVANGELISTA DOS SANTOS JUNIOR",
      "JOAO EVANGELISTA DOS SANTOS JR",
      "JOAO EVANGELISTA",
    ],
  ...
  },
  {
    ...
    address:
      "Av. Augusto de Campos, 351 - Jardim das Estacoes (Vila Xavier), Araraquara - SP, 14810-349",
    lat: -21.7746627,
    lng: -48.1447338,
  },
  {
  ...
  },
  {
    ...
    address:
      "Rua Henrique Bráulio de Melo Sobrinho, Jardim Santa Luzia, São José dos Campos - SP, CEP 12228-850",
    lat: -23.2422009,
    lng: -45.8357956,
  },
  {
...
function fixedAddressLocality(
  address: string,
): Pick<Technician, "cityOriginal" | "cityNormalized" | "state"> {
...
export function applySeedAddresses(records: Technician[]): Technician[] {
  ...
  const results = records.map((t) => {
    // Técnicos cadastrados explicitamente (via Supabase) carregam o próprio
    // endereço/coordenadas exatos. O seed não deve movê-los para um endereço
    // fixo só porque o primeiro nome coincide com uma keyword da lista.
    if ((t.id || "").startsWith("supabase_")) {
      console.log(`%c⏭ SKIP SEED para técnico cadastrado: "${t.nameOriginal}"`, "color: #2563eb;");
      return t;
    }
    ...
    if (matchedSeed) {
      ...
      return {
        ...
        addressLat: isInvalid
          ? undefined
          : (matchedSeed.lat ?? (alreadyUsingFixedAddress ? t.addressLat : undefined)),
        addressLng: isInvalid
          ? undefined
          : (matchedSeed.lng ?? (alreadyUsingFixedAddress ? t.addressLng : undefined)),
      };
    }

    console.warn(`%c❌ SEM MATCH: "${t.nameOriginal}" (norm: "${techNorm}")`, "color: #dc2626;");
    return t;
  });

  const matched = results.filter(
    (r, i) => r.address !== records[i].address || r.addressLat !== records[i].addressLat,
  );
  console.log(
    `[SEED] Resumo: ${matched.length}/${records.length} técnicos com endereço real aplicado.`,
  );
  console.groupEnd();
  return results;
}
