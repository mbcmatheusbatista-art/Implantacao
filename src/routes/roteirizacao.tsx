...
function RoteirizacaoPage() {
  ...
  const enrichedTechnicians = useMemo(
    () => {
      ...
      // Always show the supplied fixed-location register, even when the
      // browser still has only the old single-record local seed.
      const fixedOnly = getSeedTechnicians().filter((technician) => {
