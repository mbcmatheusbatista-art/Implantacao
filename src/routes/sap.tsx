...
function SapPage() {
  ...
  return <div className="space-y-3 max-w-[1400px] mx-auto" onPaste={handlePaste}>
    ...
    {contextOpen && contextItem && <div className="fixed z-50 w-52 max-w-[calc(100vw-1rem)] rounded-md border bg-background p-1 shadow-lg" style={contextPosition} onClick={(e) => e.stopPropagation()}><Button variant="ghost" className="h-8 w-full justify-start px-2 text-sm" onClick={() => openEditor(contextItem)}>Editar tabela</Button><Button variant="ghost" className="h-8 w-full justify-start px-2 text-sm" onClick={() => rename(contextItem)}>Renomear atalho</Button><Button variant="ghost" className="h-8 w-full justify-start px-2 text-sm" onClick={() => duplicate(contextItem)}>Duplicar atalho</Button><Button variant="ghost" className="h-8 w-full justify-start px-2 text-sm" onClick={() => { download(`${contextItem.name}.tsv`, matrixToTsv(contextItem.dataMatrix), "text/tab-separated-values"); setContextOpen(false); }}>Exportar TSV</Button><Button variant="ghost" className="h-8 w-full justify-start px-2 text-sm" onClick={() => { download(`${contextItem.name}-backup.json`, JSON.stringify(contextItem, null, 2), "application/json"); setContextOpen(false); }}>Exportar backup JSON</Button><Button variant="ghost" className="h-8 w-full justify-start px-2 text-sm text-destructive hover:text-destructive" onClick={() => remove(contextItem)}>Excluir atalho</Button></div>}
